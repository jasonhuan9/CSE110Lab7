describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await product.evaluate(el => el.shadowRoot.querySelector('button').click());
    const buttontxt = await product.evaluate(el => el.shadowRoot.querySelector('button').innerText);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartNum = await page.$eval('#cart-count', el => el.textContent);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    expect(JSON.parse(cartData)).toEqual(Array.from({length: 20}, (_, i) => i + 1));
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (const product of products) {
        const buttontxt = await product.evaluate(el => el.shadowRoot.querySelector('button').innerText);
        expect(buttontxt).toBe('Add to Cart');
    }
